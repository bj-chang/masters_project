...
\begin{itemize}
  \item experiments use the Python package \texttt{meshdep}
    (Firedrake~\citep{firedrake2016} + pyadjoint~\citep{mitusch2019})
  \item Poisson optimal control problem exposed through
    \texttt{meshdep.problems.poisson\_control.solve\_poisson\_control}
  \item accepts \texttt{riesz\_map} argument: \texttt{"l2"},
    \texttt{"L2"}, or \texttt{"H1"}; passed straight to pyadjoint's
    \texttt{Control}:
...
\end{lstlisting}
  \item keyword chooses which discrete Riesz map pyadjoint applies
    when converting dual derivative $\mathrm{d}\Jt(m)$ to primal gradient
  \item forward Poisson solve uses LU $\Rightarrow$ discrete gradient
    computed to machine precision $\Rightarrow$ convergence threshold
    $\varepsilon = 10^{-7}$ on $\|\Rmap_H(J')\|_H$ not blocked by
    Krylov-solver noise
\end{itemize}
...
algorithm-specific.}
\begin{itemize}
  \item \citet[p.~67-68]{shfp2017} stop every algorithm in a panel
    on the same gradient norm $\|\Rmap_H(J')\|_H \le 10^{-7}$,
    regardless of algorithm's internal inner product
  \item for $\ell^{2}$ algorithms (e.g.\ SciPy L-BFGS-B):
    \citet{shfp2017} compute the derivative w.r.t.\ the appropriate
    inner product \emph{outside} the algorithm
  \item reproduced in \texttt{meshdep.optimisers.solve\_with\_scipy\_external\_check}:
    \begin{itemize}
      \item run SciPy's L-BFGS-B with native tolerances $10^{-30}$
        (so they never trigger)
      \item callback re-evaluates $\Rmap_H(J')$ at current iterate,
        computes $H$-norm, raises sentinel exception when threshold met
    \end{itemize}
\end{itemize}
...
\begin{itemize}
  \item natural recipe: \texttt{Control(m, riesz\_map="L2")} +
    pyadjoint's TAO wrapper
  \item pyadjoint installs $M_h^{-1}$ as
    \begin{itemize}
      \item gradient-norm metric in convergence test
        (\texttt{tao.setGradientNorm})
      \item initial Hessian $B_0$ in L-BFGS recursion
        (\texttt{setLMVMH0})
    \end{itemize}
  \item LMVM Mat then maintains secant pairs in the inner product
    induced by $B_0$ $\Rightarrow$ in principle doing function-space
    L-BFGS
...
\begin{itemize}
  \item \texttt{tao\_lmvm\_mat\_lmvm\_hist\_size} (controls number
    of secant pairs kept) is silently ignored when passed through
    the pyadjoint \texttt{parameters} dict
  \item must register on global PETSc options database \emph{before}
    \texttt{TAOSolver} is built:
\begin{lstlisting}[language=Python]
...
\begin{itemize}
  \item rank-five quasi-Newton update only captures top-5
    eigenvalues of discrete Hessian
  \item on graded meshes (\cref{tab:meshdep:mesh_summary}) the
    Hessian has a long-tailed spectrum $\Rightarrow$ poorly
    conditioned search direction
  \item slow convergence, not outright failure: with enough outer
    iters, gradient norm \emph{does} reach machine precision (probe
    in \cref{sec:meshdep:tao-revisited})
  \item iteration count at $\varepsilon = 10^{-7}$ grows from $\sim 130$
    (Cartesian) to several hundred (high grading)
  \item fix: history $= 100$ $\Rightarrow$ mesh-independent
    iteration count of $\sim 30$ across the range
\end{itemize}

\paragraph{A transparent Hilbert-space L-BFGS reference
implementation.}
\begin{itemize}
  \item TAO/LMVM with the corrections above reproduces Schwedes'
    pattern, but the rank-two update lives inside a PETSc C kernel
    $\Rightarrow$ hard to inspect from Python
  \item to make the inner product completely explicit, implement
    BFGS-in-Hilbert-space directly (following
    \citet[Sect.~1.5.4.1]{shfp2017}):
    \texttt{meshdep.optimisers.solve\_with\_hilbert\_lbfgs}
  \item every dot product in the two-loop recursion uses the chosen
    $\inner{\cdot}{\cdot}{H}$ instead of $\ell^{2}$
  \item this is the distinguishing feature of Schwedes' Moola row;
    algorithm given in \cref{sec:meshdep:hilbert-lbfgs}
\end{itemize}
...
\end{equation}
  \item current iterate $m_{k} \in \Mspace$, primal Riesz
...
\end{equation}
  \item standard L-BFGS rescaling \citep[Sect.~1.5.4.2]{shfp2017},
    adapts $B_0$ to local curvature
  \item empirically: omitting this $\Rightarrow$ algorithm plateaus
    several orders above threshold even on a well-conditioned mesh
\end{itemize}

\paragraph{Line search and convergence.}
\begin{itemize}
  \item backtracking Armijo: start $\alpha = 1$, halve until
    sufficient-decrease
\begin{equation}
  \Jt(m_{k} + \alpha\, p_{k})
    \le \Jt(m_{k})
  ...
  \qquad c_{1} = 10^{-4}
\end{equation}
  \item curvature pairs with $\inner{s_{j}}{y_{j}}{H} \le 0$ silently
    dropped
...
\begin{itemize}
  \item $g_k$ obtained from dual derivative via direct LU on the
    inner-product matrix ($M_h$ for $L^2$, $M_h + K_h$ for $H^1$)
  \item default Firedrake \texttt{RieszMap} uses Krylov $\Rightarrow$
    extra solver tolerance leaks into convergence test
  \item direct factorisation removes that tolerance $\Rightarrow$ any
    failure to reach $\varepsilon = 10^{-7}$ is the optimiser's
    fault, not the Riesz solver's
\end{itemize}
...
\begin{itemize}
  ...
  \item setup:
    \begin{itemize}
      ...
      \item objective: \cref{eq:objective-J} with
        $d(x, y) = \sin(\pi x) \sin(\pi y)$ and $\alpha = 10^{-4}$
      \item control init: $m \equiv 0$
      \item convergence threshold: $\varepsilon = 10^{-7}$ on
        $\|\Rmap_{L^2}(J')\|_{L^{2}}$
      \item L-BFGS history of 5 for both algorithms
    \end{itemize}
  \item two algorithms run:
    \begin{itemize}
      \item SciPy L-BFGS-B on the $\ell^{2}$ coefficient vector via
        \texttt{meshdep.optimisers.solve\_with\_scipy\_external\_check},
        convergence checked externally in $L^{2}$
      \item Hilbert-space L-BFGS of \cref{sec:meshdep:hilbert-lbfgs}
        with $H = \Lp{\Om}$ via
        \texttt{meshdep.optimisers.solve\_with\_hilbert\_lbfgs}
    \end{itemize}
  \item results in \cref{tab:meshdep:panel-a}: clear mesh-dependent
    growth of $\ell^{2}$ row, flat $L^{2}$ row $\Rightarrow$
    consistent with H1 and H2 of \cref{sec:meshdep:hypothesis}
\end{itemize}
...
\begin{itemize}
  \item $\ell^{2}$ row nearly doubles at every refinement
    ($\times 2.09, \times 2.03, \times 1.93, \times 2.08$)
    $\Rightarrow$ matches polynomial-in-$h_{\max}/h_{\min}$ growth
    of \citet[Sect.~2.2.4]{shfp2017}
  \item $L^{2}$ row fluctuates between 7 and 8 iters across
    20$\times$ change in realised mesh ratio
  \item Schwedes Panel (a) reference: \citet[Table~2.2(a)]{shfp2017}
    reports $27, 47, 75, 97, 133$ (SciPy) and $22, 20, 22, 23, 23$
    (Moola) at the same target ratios
  \item qualitative pattern reproduces; absolute counts differ
    because:
    \begin{itemize}
      \item realised $h_{\max}/h_{\min}$ on our meshes larger than
        nominal target (cf.~\cref{tab:meshdep:mesh_summary})
      \item our backtracking Armijo line search converges in fewer
        outer iters than Moola's strict-Wolfe
    \end{itemize}
  \item extra run: Hilbert L-BFGS at $r = 128$ (realised $193.88$,
    $141\,138$ cells) $\Rightarrow$ 9 iters; flat behaviour extends
    beyond the Schwedes range
\end{itemize}
...
\begin{itemize}
  \item \cref{sec:meshdep:implementation} said: TAO/LMVM with
    \texttt{Control(m, riesz\_map="L2")} fails to reproduce Moola
    row; fix = larger LMVM history
  \item this subsection: the diagnostic chain that led to this fix
    (was misdiagnosed once already)
\end{itemize}

\paragraph{Hypothesis 1: noise floor (ruled out).}
\begin{itemize}
  \item first guess: LMVM stagnating at a gradient-norm floor below
    which adjoint-tape round-off prevents further reduction
  \item test: replace TAO/LMVM with TAO/NLS (exact Newton w/ CG-based
    Hv products), tighten tolerance to $\varepsilon = 10^{-30}$
  \item Poisson is quadratic $\Rightarrow$ one Newton step lands at
    discrete optimum (modulo Newton solver tolerance)
  \item evaluate gradient at that iterate: $\|\Rmap_{L^{2}}(J')\|_{L^{2}}
    \approx 10^{-18}$ on every mesh in sweep
  \item $\Rightarrow$ adjoint-tape noise floor at machine precision,
    not at the $10^{-5}$ where LMVM stagnated
  \item conclusion: not a noise floor
\end{itemize}

\paragraph{Hypothesis 2: limited-memory approximation (confirmed).}
\begin{itemize}
  \item next guess: rank-5 quasi-Newton update too coarse for graded
    Hessian spectrum
  \item test: same artificially tight $\varepsilon = 10^{-30}$, budget
    $3000$ iters, sweep history $h \in \{5, 25, 50, 100, 200\}$
  \item results in \cref{tab:meshdep:tao-history}:
    \begin{itemize}
      \item $h=5$, uniform mesh: reaches $10^{-13}$
      \item $h=5$, stretch 5 mesh: stagnates eleven orders higher
      \item $h=25$ already brings both to machine precision
      \item further increases change final norm only marginally
    \end{itemize}
  \item conclusion: rank-5 update too coarse for the long-tailed
    spectrum on graded meshes; with enough memory, LMVM reaches the
    discrete minimum to machine precision on every mesh
\end{itemize}
...
\begin{itemize}
  \item setting \texttt{tao\_lmvm\_mat\_lmvm\_hist\_size} via
    pyadjoint's \texttt{parameters} dict does not propagate to the
    LMVM sub-Mat
  \item cause: options-prefix mismatch between pyadjoint and the
    internal PETSc matrix object attached to the LMVM Tao
  \item fix: register on global PETSc options DB \emph{before}
    constructing \texttt{TAOSolver}:
\begin{lstlisting}[language=Python]
PETSc.Options().setValue('-tao_lmvm_mat_lmvm_hist_size', 100)
\end{lstlisting}
  \item alternative: construct pyadjoint TAO solver with empty
    \texttt{options\_prefix}
  \item we use the former, see \cref{sec:meshdep:implementation}
\end{itemize}

\paragraph{Does this generalise to a non-quadratic problem?}
\begin{itemize}
  \item Poisson is quadratic in $m$; quasi-Newton methods are
    unusually effective on quadratics regardless of inner product
    used $\Rightarrow$ check on a non-quadratic problem
  \item repeat experiment on 1D viscous Burgers control problem via
    \texttt{meshdep.problems.burgers\_control.solve\_burgers\_control}
  \item forward equation (backward Euler):
\begin{equation}
  u_{t} + u\, u_{x} - \nu\, u_{xx} = m,
  \qquad x \in (0, 1),\ t \in (0, T)
\end{equation}
    with $\nu = 10^{-3}$ and homogeneous Dirichlet BCs; objective
    penalises misfit at terminal time
  \item sweep the same stretching parameter as Poisson, TAO/LMVM
    with default history of 5
  \item three configurations:
    \begin{itemize}
      \item $L^{2}$ Riesz, $\varepsilon = 10^{-7}$
      \item $L^{2}$ Riesz, $\varepsilon = 10^{-5}$ (loose)
      \item $H^{1}$ Riesz, $\varepsilon = 10^{-3}$ (fair)
    \end{itemize}
\end{itemize}
...
\begin{itemize}
  \item headline: the Poisson LMVM blow-up does \emph{not} transfer
    to Burgers $\Rightarrow$ LMVM iteration count flat across the
    grading sweep in all three configurations
  \item but the picture is solver-dependent
    (see \cref{tab:meshdep:tao-zoo-burgers}):
    \begin{itemize}
      \item Newton-class solvers + \texttt{lmvm}: mesh-independent
        on Burgers, same as on Poisson with the history fix
      \item \texttt{cg}: \emph{is} still mesh-dependent on Burgers
        ($104 \to 438 \to $ fail) - the blow-up transfers
    \end{itemize}
  \item explanation:
    \begin{itemize}
      \item graded-mesh Poisson Hessian has long-tailed spectrum
        because the discrete Laplacian is very ill-conditioned on
        anisotropic grids
      \item Burgers Hessian better conditioned for Newton-class and
        LMVM: implicit time stepping acts as mass-matrix
        preconditioner on high-freq modes
      \item but time-stepping preconditioning does \emph{not} rescue
        \texttt{cg}, which has neither Hessian information nor a
        separate preconditioner
    \end{itemize}
  \item refined conclusion: Schwedes' Table 2.2(a) mesh-dependence
    is really about \emph{solver classes that don't capture enough
    Hessian information}, on graded meshes for diffusion-dominated
    problems - not a universal $\ell^{2}$ pathology of every
    iterative solver
\end{itemize}

\paragraph{Survey across other TAO solvers.}
\begin{itemize}
  \item same Poisson problem + grading sweep, run through every
    PETSc TAO solver that accepts a (possibly bound-constrained)
    unconstrained problem of this class
  \item summarised in \cref{tab:meshdep:tao-zoo}
  \item Newton-class solvers + \texttt{lmvm} with history fix:
    mesh-independent
  \item \texttt{cg} shows the same mesh-dependent growth as LMVM
    at default history $\Rightarrow$ pathology is not specific to
    LMVM
\end{itemize}
...
\begin{table}[H]
  ...
  \begin{tabular}{l l r r r r}
    \toprule
    type & TAO solver
       & \multicolumn{4}{c}{realised $h_{\max} / h_{\min}$} \\
    \cmidrule(lr){3-6}
       & & $1.0$ & $18.3$ & $48.2$ & $126.9$ \\
    \midrule
    Newton line search                & \texttt{nls}   & $2$ & $2$ & $2$ & $2$ \\
    bounded Newton line search        & \texttt{bnls}  & $1$ & $1$ & $1$ & $1$ \\
    bounded Newton trust region       & \texttt{bntr}  & $8$ & $8$ & $7$ & $7$ \\
    bounded Newton trust + line       & \texttt{bntl}  & $8$ & $8$ & $7$ & $7$ \\
    bounded Newton Krylov             & \texttt{bnk}   & \multicolumn{4}{c}{\textit{n/a}} \\
    quasi-Newton (LMVM, history $100$) & \texttt{lmvm}  & $4$ & $18$ & $27$ & $28$ \\
    bounded quasi-Newton + line       & \texttt{bqnls} & $4$ & $24$ & $31$ & $26$ \\
    nonlinear conjugate gradient      & \texttt{cg}    & $37$ & $204$ & $100$ & $386$ \\
    orthant-wise quasi-Newton         & \texttt{owlqn} & \multicolumn{4}{c}{\textit{n/a}} \\
    \bottomrule
  \end{tabular}
  \caption{TAO solver iteration counts on the Poisson optimal
  control problem of \cref{sec:meshdep:panel-a}, at convergence
  threshold $\varepsilon = 10^{-7}$ on
  $\|\Rmap_{L^{2}}(J')\|_{L^{2}}$. \texttt{lmvm} uses the
  \cref{sec:meshdep:implementation} fix
  ($-tao\_lmvm\_mat\_lmvm\_hist\_size = 100$). \texttt{bnk} requires
  explicit bound vectors and so cannot run on the unbounded problem.
  \texttt{owlqn} is designed for $L^{1}$-regularised objectives;
  it reports immediate convergence at $m = 0$ under its $L^{1}$
  optimality condition, which is not the optimum of the $L^{2}$
  Tikhonov problem.}
...
\begin{itemize}
  \item same survey on the 1D Burgers control problem of
    \cref{tab:meshdep:burgers}, across the same grading sweep
  \item summarised in \cref{tab:meshdep:tao-zoo-burgers}
  \item Newton-class + \texttt{lmvm}: all mesh-independent (absolute
    counts higher than Poisson because Burgers is non-quadratic, but
    flat across stretches)
  \item \texttt{cg}: still mesh-dependent ($104 \to 438$, then runs
    out of budget at higher grading) $\Rightarrow$ time-stepping
    preconditioning rescues Newton-class and LMVM but not
...
\begin{table}[H]
  ...
  \begin{tabular}{l l r r r r}
    ...
    \cmidrule(lr){3-6}
       & & $1.0$ & $6.9$ & $18.3$ & $48.2$ \\
    \midrule
    Newton line search                & \texttt{nls}   & $4$ & $4$ & $4$ & $3$ \\
    bounded Newton line search        & \texttt{bnls}  & $7$ & $5$ & $5$ & $5$ \\
    bounded Newton trust region       & \texttt{bntr}  & $9$ & $8$ & $8$ & $7$ \\
    bounded Newton trust + line       & \texttt{bntl}  & $9$ & $8$ & $8$ & $7$ \\
    bounded Newton Krylov             & \texttt{bnk}   & \multicolumn{4}{c}{\textit{n/a}} \\
    quasi-Newton (LMVM, history $100$) & \texttt{lmvm}  & $87$ & $80$ & $78$ & $72$ \\
    bounded quasi-Newton + line       & \texttt{bqnls} & $41$ & $41$ & $34$ & $34$ \\
    nonlinear conjugate gradient      & \texttt{cg}    & $104$ & $438$ & \textit{fail} & \textit{fail} \\
    orthant-wise quasi-Newton         & \texttt{owlqn} & \multicolumn{4}{c}{\textit{n/a}} \\
    \bottomrule
  \end{tabular}
  \caption{TAO solver iteration counts on the 1D Burgers control
  problem of \cref{tab:meshdep:burgers}, at convergence threshold
  $\varepsilon = 10^{-7}$ on $\|\Rmap_{L^{2}}(J')\|_{L^{2}}$.
  \textit{fail} means the solver did not converge within the budget
  of $1000$ iterations. \texttt{lmvm} uses the
  \cref{sec:meshdep:implementation} fix. \texttt{bnk} and
  \texttt{owlqn} are not applicable for the reasons given in
  \cref{tab:meshdep:tao-zoo}.}
  \label{tab:meshdep:tao-zoo-burgers}
\end{table}
...
\begin{itemize}
  \item Panel (a) reproduced central finding of
    \citet[Sect.~2.3]{shfp2017}:
    \begin{itemize}
      \item function-space inner product respected $\Rightarrow$
        quasi-Newton iteration count essentially mesh-independent
      \item ambient $\ell^{2}$ inner product $\Rightarrow$ count
        grows at the polynomial rate predicted in
...
\textbf{1. \texttt{riesz\_map} on \texttt{Control} \emph{is} enough
for TAO/LMVM.}
\begin{itemize}
  \item contrary to a conjecture that an earlier version of this
    chapter recorded: the LMVM Mat \emph{does} respect the inner
    product induced by the initial Hessian preconditioner in its
    rank-two updates
  \item the original failure to reproduce the Moola row was caused
    by the limited-memory approximation being insufficient, not by
    an internal $\ell^{2}$ inner product
  \item diagnostic chain in \cref{sec:meshdep:tao-revisited}:
    \begin{itemize}
      \item LMVM history sweep at tight tolerance: final gradient
        norm on graded mesh falls from $10^{-11}$ at $h = 5$ to
        machine precision at $h = 25$
      \item on Burgers, default history is already sufficient
    \end{itemize}
  \item practical recipe for reproducing the Moola row via stock
    TAO/LMVM:
    \begin{itemize}
      \item construct \texttt{Control(m, riesz\_map="L2")}
      \item set \texttt{tao\_lmvm\_mat\_lmvm\_hist\_size} to a value
        that captures the relevant part of the discrete Hessian
        spectrum
    \end{itemize}
\end{itemize}
...
\begin{itemize}
  ...
  \item but the hand-written Hilbert L-BFGS of
    \cref{sec:meshdep:hilbert-lbfgs} is useful as a reference
    implementation:
    \begin{itemize}
      \item every dot product in the two-loop recursion is explicit
        in the Python source
      \item straightforward to verify where the chosen inner product
        enters
    \end{itemize}
  \item also dodges the pyadjoint options-prefix subtlety that makes
    the LMVM history hard to set via the public \texttt{parameters}
    interface (see \cref{sec:meshdep:tao-revisited})
\end{itemize}

\textbf{3. Panel (b) discrepancy with Schwedes - unresolved.}
\begin{itemize}
  \item our Panel (b) iteration count is much higher than
    \citet[Table~2.2(b)]{shfp2017}
  \item Hilbert L-BFGS converges to the correct minimum but takes
    two orders of magnitude more iters than the book reports
  \item attributable to:
    \begin{itemize}
      \item primary: convergence threshold expressed in $H^{1}$
        (stricter norm than $L^{2}$)
      \item secondary: high condition number of the discrete Hessian
        in $H^{1}$ on the unit square
    \end{itemize}
  \item routing the same \texttt{Control(m, riesz\_map="H1")} through
    TAO/LMVM with a mesh-fair $H^{1}$ tolerance: $\sim 25$ iters
    across grading sweep $\Rightarrow$ $H^{1}$ Riesz map itself
    does deliver mesh independence
  \item identifying the 4-iter Moola step needs Moola source
    inspection; not possible in our Firedrake environment
\end{itemize}
