...
\texttt{pdeopt.meshes} encapsulates the recipe. The Netgen Python
...
The numerical experiments use the Python package \texttt{pdeopt}
...
is exposed through
\texttt{pdeopt.problems.poisson\_control.solve\_poisson\_control},
...
\texttt{pdeopt.optimisers.solve\_with\_scipy\_external\_check}, which
...
\paragraph{Setting \texttt{riesz\_map} on \texttt{Control} is
necessary, but not sufficient for PETSc TAO/LMVM to behave like
Moola.} The natural attempt to reproduce the Moola row of
Table~2.2 in \citet{shfp2017} is to construct
...
\texttt{setLMVMH0}). The internal BFGS update formula in PETSc TAO,
however, accumulates curvature pairs using the standard $\ell^{2}$
dot product on coefficient vectors: it does not respect the chosen
Hilbert space inner product in its rank-two updates. On the Netgen
meshes of \cref{tab:meshdep:mesh_summary}, the resulting algorithm
stalls at a sub-optimal iterate where TAO's strong-Wolfe line search
refuses to take further steps. The mesh-independent behaviour of the
Moola row in \citet[Table~2.2(a)]{shfp2017} is therefore not
recovered by changing the \texttt{Control} alone; a more fundamental
change to the optimisation algorithm is required.

\paragraph{We implement BFGS in a Hilbert space directly.} Following
\citet[Sect.~1.5.4.1]{shfp2017}, the function
\texttt{pdeopt.optimisers.solve\_with\_hilbert\_lbfgs} implements an
L-BFGS algorithm in which every dot product used inside the two-loop
recursion uses the chosen Hilbert space inner product
$\inner{\cdot}{\cdot}{H}$ rather than the ambient $\ell^{2}$ inner
product on coefficient vectors. This is the distinguishing feature
of Schwedes' ``Moola'' row, and is the missing ingredient in the
PETSc TAO/LMVM attempt described above. The algorithm is presented
in \cref{sec:meshdep:hilbert-lbfgs}.
...
default Firedrake \texttt{RieszMap} uses a Krylov solver with
relative tolerance $10^{-5}$, which would otherwise put a noise
floor of order $10^{-5}$ on the primal gradient and prevent the
algorithm from reaching the convergence threshold.
...
\begin{itemize}
  \item SciPy's L-BFGS-B run on the $\ell^{2}$ coefficient vector
        through
        \texttt{pdeopt.optimisers.solve\_with\_scipy\_external\_check},
  ...
  \item the Hilbert-space L-BFGS of
        ...
        $H = \Lp{\Om}$ through
        \texttt{pdeopt.optimisers.solve\_with\_hilbert\_lbfgs}.
\end{itemize}
...
Reading the spectrum of the Hessian of $\Jt$ in the $H^{1}$ inner
product, this slow convergence is not entirely surprising. The
exact-arithmetic eigenvalues of $(I - \Delta)^{-1}$ on the unit
square scale as $1/(1 + \pi^{2}(j^{2} + k^{2}))$, and composing this
with the $L^{2}$-Hessian of $\Jt$ produces a transformed Hessian
with eigenvalues decaying as $k^{-6}$ on the high-frequency
eigenfunctions. The condition number is therefore large, and a
limited-memory BFGS with history five cannot resolve the
high-frequency modes in a constant number of iterations.

The four-iteration count reported by \citet{shfp2017} for the
$H^{1}$ Moola row is not explained in the book, and we have not yet
identified a Moola-specific preconditioning step that would
reproduce it from the algorithmic ingredients described in
\citet[Sect.~1.5.4.1]{shfp2017}. We leave this as an open question:
either Moola applies an additional preconditioner that is not
documented in the book (such as solving the Hessian's compact term
explicitly), or its iteration count is defined relative to a
...
Two observations from the implementation work bear on the practical
use of optimisation packages catalogued by \citet{shfp2017}. First,
attaching a \texttt{riesz\_map} to a pyadjoint \texttt{Control} is
necessary but not sufficient for PETSc TAO/LMVM to reproduce the
behaviour of \citet{shfp2017}'s Moola row, because the BFGS update
formula in PETSc TAO uses an $\ell^{2}$ dot product on coefficient
vectors internally and does not consult the Riesz map outside the
initial Hessian and the convergence test. To recover the
mesh-independent behaviour with a stock optimisation library would
require a library that supports user-defined inner products
throughout the algorithm; in the absence of such a library we
implemented BFGS-in-Hilbert-space directly, following the formulas
of \citet[Sect.~1.5.4.1]{shfp2017}.

Second, the discrepancy between our Panel (b) results and those
reported by \citet[Table~2.2(b)]{shfp2017} indicates that the
discussion in the reference text does not fully specify how the
Moola row is computed in the $H^{1}$ case. Our implementation
follows the formulas given in the book and converges to the correct
minimum of the discrete objective, but takes two orders of magnitude
more iterations than the value reported in the book. We do not
believe this is a defect of the implementation: the convergence
trajectory is monotone in the discrete objective value and reaches
the same minimiser as SciPy. We conjecture that Moola applies an
additional algorithmic step in the $H^{1}$ case that is not
described in \citet[Sect.~1.5.4]{shfp2017}, and we leave the
identification of this step as future work.
...
\citet[Sect.~2.2.4]{shfp2017}. The route of changing only the
\texttt{Control}'s \texttt{riesz\_map} keyword and routing the
problem through PETSc TAO/LMVM was found to be insufficient: PETSc
TAO uses an $\ell^{2}$ dot product internally in its BFGS update
formula, irrespective of which Riesz map is installed as the
initial Hessian preconditioner. Panel~(b) of Table~2.2 was only
...
on the implementation side, is to extend the \texttt{pdeopt} package
